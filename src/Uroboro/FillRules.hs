...
fillRules prog (EST.Def _ abss (EST.FunDefNature (EST.FunSig _ estId _ _ _) ruls))
    ...
    where 
        ...
        mismatchCop prg locr copId -- not needed if mismatch' from FillSigma is used
            | estId /= copId = tyErrIdMismatch locr (show estId) (show copId)
            | otherwise = Right prg        
...
zipStrict :: Location -> [a] -> [b] -> Either Error [(a,b)]
zipStrict loc as bs 
    | length as == length bs = return $ zip as bs
    | otherwise              = tyErrLenMismatch loc


zipArgsWithItsTypes :: Location -> (Context -> Type -> a -> Either Error (b,Context)) -> Context -> [a] -> [Type] -> Either Error ([b], Context)          
zipArgsWithItsTypes loc f ctx as ts = do
    asts <- zipStrict loc as ts
    foldM (\(ps,c) (a,t) -> do (p,c2) <- f c t a; return (p:ps,c2)) ([], ctx) asts 

zipArgsWithItsTypesMap :: Location -> (Type -> a -> Either Error b) -> [a] -> [Type] -> Either Error [b]        
...
replaceByIdAps sgm ident apss = 
    case (getTypInfosForId sgm ident, getLookup1For sigAbs sgm ident) of
      ...
      (_,_) -> tyErrUnbound (idLoc ident) (idId ident)
...
checkIsSameType :: Location -> Sigma -> Context -> Type -> Type -> Either Error Sigma
checkIsSameType loc sgm ctx expTy actTy
    = case (expTy, actTy) of
        (TypeT _ tau1 taps1, TypeT _ tau2 taps2) -> if tau1 /= tau2
                then tyErrTyMismatch loc (show expTy) (show actTy) 
        ...
        (_,_) -> tyErrTyMismatch loc (show expTy) (show actTy) 
